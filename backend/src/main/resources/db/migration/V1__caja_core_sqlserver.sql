...
-- =========================================================
-- Triggers de validacion de integridad
-- =========================================================

CREATE TRIGGER trg_EstudianteTutor_NoDuplicados
ON Estudiante_tutor
AFTER INSERT
AS
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM (
      SELECT i.ID_estudiante, i.ID_tutor
      FROM inserted i
      GROUP BY i.ID_estudiante, i.ID_tutor
      HAVING COUNT(*) > 1
      ...
      SELECT i.ID_estudiante, i.ID_tutor
      FROM inserted i
      INNER JOIN Estudiante_tutor et
        ON et.ID_estudiante = i.ID_estudiante
       AND et.ID_tutor = i.ID_tutor
       AND et.ID_estudiante_tutor <> i.ID_estudiante_tutor
    ) x
  )
  BEGIN
    RAISERROR('Ya existe la relacion estudiante-tutor.', 16, 1);
...
CREATE TRIGGER trg_EstudianteGrupo_NoDuplicados
ON Estudiante_Grupo
AFTER INSERT
AS
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM (
      SELECT i.ID_estudiante, i.ID_Grupo
      FROM inserted i
      GROUP BY i.ID_estudiante, i.ID_Grupo
      HAVING COUNT(*) > 1
      ...
      INNER JOIN Estudiante_Grupo eg
        ON eg.ID_estudiante = i.ID_estudiante
       AND eg.ID_Grupo = i.ID_Grupo
       AND eg.ID_Estudiante_Grupo <> i.ID_Estudiante_Grupo
    ) x
  )
  BEGIN
    RAISERROR('El estudiante ya esta inscrito en ese grupo.', 16, 1);
    ROLLBACK TRANSACTION;
    RETURN;
  END
END;
...
CREATE TRIGGER trg_ProfesorGrupo_NoDuplicados
ON Profesor_Grupo
AFTER INSERT
AS
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM (
      SELECT i.ID_Profesor, i.ID_Grupo
      FROM inserted i
      GROUP BY i.ID_Profesor, i.ID_Grupo
      HAVING COUNT(*) > 1
      ...
      SELECT i.ID_Profesor, i.ID_Grupo
      FROM inserted i
      INNER JOIN Profesor_Grupo pg
        ON pg.ID_Profesor = i.ID_Profesor
       AND pg.ID_Grupo = i.ID_Grupo
       AND pg.ID_Profesor_Grupo <> i.ID_Profesor_Grupo
    ) x
  )
  BEGIN
    RAISERROR('El profesor ya esta asignado a ese grupo.', 16, 1);
    ROLLBACK TRANSACTION;
    RETURN;
  END
END;
...
CREATE TRIGGER trg_SolicitudAdmision_BloquearAprobacionSinDocumentos
...
BEGIN
  ...
  IF EXISTS (
    ...
    INNER JOIN Estado_Solicitud es
      ON es.ID_estado_solicitud = i.ID_estado_solicitud
    WHERE es.Nombre = 'APROBADA'
      AND d.ID_estado_solicitud <> i.ID_estado_solicitud
      AND EXISTS (
        SELECT 1
        FROM Tipo_Documento td
        WHERE ISNULL(td.Es_Obligatorio, 1) = 1
          AND NOT EXISTS (
            SELECT 1
            FROM Documento_Solicitud ds
            WHERE ds.ID_solicitud = i.ID_solicitud
              AND ds.ID_tipo_documento = td.ID_tipo_documento
          )
      )
  )
  BEGIN
    RAISERROR('No se puede aprobar la solicitud: faltan documentos obligatorios.', 16, 1);
    ROLLBACK TRANSACTION;
    RETURN;
  END
END;
