...
-- ==========================================
-- En T-SQL no existe FOREIGN_KEY_CHECKS de MySQL. 
-- Si necesitas limpiar datos en orden, se eliminan las restricciones o se borran las tablas en cascada.

IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'micasita')
BEGIN
    CREATE DATABASE micasita;
END
GO
...
-- Nota: Para simular "ON UPDATE CURRENT_TIMESTAMP" de MySQL en T-SQL, 
-- se suele usar un trigger AFTER UPDATE. Por defecto se inicializa con SYSDATETIME().
...
CREATE TABLE Configuracion_Finanzas (
    ID_configuracion BIGINT IDENTITY(1,1) PRIMARY KEY,
    Monto_Matricula_Base DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    Monto_Mensualidad_Base DECIMAL(10,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE Solicitud_Admision (
  ...
  Activo BIT DEFAULT 1,
  CONSTRAINT FK_SolicitudAdmision_Persona FOREIGN KEY (ID_persona) REFERENCES Persona(ID_persona),
  CONSTRAINT FK_SolicitudAdmision_EstadoSolicitud FOREIGN KEY (ID_estado_solicitud) REFERENCES Estado_Solicitud(ID_estado_solicitud)
);
...
CREATE TABLE Caja_Sesion (
  ...
  CONSTRAINT FK_CajaSesion_EstadoCaja FOREIGN KEY (ID_estado_caja) REFERENCES Estado_Caja(ID_estado_caja),
  CONSTRAINT FK_CajaSesion_UsuarioApertura FOREIGN KEY (ID_usuario_apertura) REFERENCES Usuario(ID_usuario),
  CONSTRAINT FK_CajaSesion_UsuarioCierre FOREIGN KEY (ID_usuario_cierre) REFERENCES Usuario(ID_usuario)
);
...
CREATE TABLE Secuencia_Recibo (
  ...
  CONSTRAINT FK_SecuenciaRecibo_TipoRecibo FOREIGN KEY (ID_tipo_recibo) REFERENCES Tipo_Recibo(ID_tipo_recibo)
);
...
-- Las tablas Log_Auditoria, Anuncio y Auditoria_Accesos_Sistema ya estaban bien formateadas en T-SQL estructuralmente
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Log_Auditoria')
BEGIN
    CREATE TABLE Log_Auditoria (
        ...
        campos_modificados NVARCHAR(MAX) NULL,                     
        Navegador_Cliente NVARCHAR(255) NULL                  
    );
END

IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Anuncio')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Auditoria_Accesos_Sistema')
BEGIN
    CREATE TABLE Auditoria_Accesos_Sistema (
        ID_acceso INT IDENTITY(1,1) PRIMARY KEY,
        ID_usuario INT NOT NULL,                                  
        Email_Usuario NVARCHAR(100) NOT NULL,                     
        Fecha_Ingreso DATETIME2 DEFAULT SYSDATETIME(),    
        IP_Terminal NVARCHAR(45) NOT NULL,                        
        Navegador_Cliente NVARCHAR(255) NULL,                     
        Estado_Intento NVARCHAR(20) NOT NULL,                     
...
-- =========================================================================
-- TRIGGERS DE AUDITORÍA (MANTENIENDO TU LÓGICA EN T-SQL NATURAl)
-- =========================================================================
